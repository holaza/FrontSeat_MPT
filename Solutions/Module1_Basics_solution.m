...
% MPC setup
% --------- Start Modifying Code Here -----------
Q = eye(2);                                % quadratic penalty x'*Q*x
R = 1;                                % quadratic penalty u'*R*u
...
% --------- Start Modifying Code Here -----------
loop = ClosedLoop(mpc,model);            % created closed-loop system
% --------- End Modifying Code Here -----------
...
[u_empc, feasible_empc, openloop_empc] = empc.evaluate(x0)
% --------- End Modifying Code Here -----------
...
% --------- Start Modifying Code Here -----------
time_empc = timeit(@() empc.evaluate(x0)); % Compute explicit solution
time_mpc = timeit(@() mpc.evaluate(x0));   % Compute numerical solution
% --------- End Modifying Code Here -----------
...
%
% A: Because evaluation of explicit policy requires only searching values 
% in a precalculated table (i.e., no optimisation, just simple mathematics)
% On the other hand, implicit MPC need to solve the MPC optimisation
% problem what is (generally) more computationaly extensive.
...
% A: The reason is that in Task 1a we have defined also constraints on both
% states to [-5 5] as well as constraints on control action [-1 1]. The
% problem is that when the position (x1) is close to the limit (e.g. 5) 
% then the speed (x2) can not be large (close to 5) as there will not exist 
% a sequence of control actions that will stop the car from hitting the
% limit (x1 -> 5). However, the car can be on the edge of hte position
% limit (x1 == 5), but its speed has to be negative such that car will go 
% towards the other edge (x1 -> -5). 
...
% --------- Start Modifying Code Here -----------
loop_mpc = ClosedLoop(mpc, model);
loop_empc = ClosedLoop(empc, model);
Nsim = 20;
x0 = [2;0];
...
% A: Yes, e.g. initial condition x0 = [-5; 4] does not converge to the origin.
...
N = 10;      % we have increased the prediction horizon from 3 -> 10
% --------- End Modifying Code Here -----------
empc = EMPCController(model, N);
empc.clicksim()


% If, based on the clicksim analysis, the system will look stable, try to
...
legend('original x1','original x2','modified x1','modified x2')
subplot(2, 1, 2)
hold on
...
% A: The construction of Lyapunov funciton failed => the syste is
%    (propably) unstable. 
